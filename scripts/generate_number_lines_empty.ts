// scripts/generate_number_lines_empty.ts
...
type Params = {
  ...
  showExample?: boolean;  // 是否显示示例（第一个问题带示例说明）
};
...
function toTex(problems: Problem[], maxValue: number, showExample?: boolean): string {
  // 一行一个算式：左边空数字线，右边显示计算式（如 2 + 3 = ___）
  return problems.map((q, i) => {
    const macro = `\\WSNumberLineAddDraw{${q.left}}{${q.right}}{${maxValue}}`;
    // 在算式之间添加虚线分隔（最后一个不加）
...
function main() {
  ...
  const params: Params = {
    ...
    showExample: Boolean(argv.showExample ?? false),  // 默认不显示示例
  };
  ...
  fs.writeFileSync("generated/problems_number_lines_empty.tex", problemsTex);
  fs.writeFileSync("generated/answers_number_lines_empty.tex", toAnswersTex(problems));

  // 计算实际使用的 seed（如果未指定则使用随机值）
  const actualSeed = params.seed !== undefined ? params.seed : Date.now();
  const meta = {
    topic: "adding-with-number-lines-empty",
  ...
  fs.writeFileSync("generated/build_meta_number_lines_empty.json", JSON.stringify(meta));
  console.log("Generated: problems_number_lines_empty.tex, answers_number_lines_empty.tex, build_meta_number_lines_empty.json");
}
