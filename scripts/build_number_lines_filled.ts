...
// Load PDF name from build_meta_number_lines_filled.json
function getPdfName(suffix?: string, outputPath?: string): string {
  try {
    const raw = fs.readFileSync("generated/build_meta_number_lines_filled.json", "utf8");
    ...
    const baseName = suffix ? `worksheet_number_lines_filled_${suffix}` : "worksheet_number_lines_filled";
...
function main() {
  ...
  const problemsPath = path.join("generated", "problems_number_lines_filled.tex");
  const answersPath = path.join("generated", "answers_number_lines_filled.tex");
  ...
  const sourcePath = path.join("dist", "worksheet_number_lines_filled.pdf");
  ...
  execSync(`tectonic templates/worksheet_number_lines_filled.tex --outdir=${distPath}`, { 
    stdio: "inherit"
  });
