...
// Load PDF name from build_meta_number_lines_empty.json
function getPdfName(suffix?: string, outputPath?: string): string {
  try {
    const raw = fs.readFileSync("generated/build_meta_number_lines_empty.json", "utf8");
    ...
    const baseName = suffix ? `worksheet_number_lines_empty_${suffix}` : "worksheet_number_lines_empty";
...
function main() {
  ...
  const problemsPath = path.join("generated", "problems_number_lines_empty.tex");
  const answersPath = path.join("generated", "answers_number_lines_empty.tex");
  ...
  const sourcePath = path.join("dist", "worksheet_number_lines_empty.pdf");
  ...
  execSync(`tectonic templates/worksheet_number_lines_empty.tex --outdir=${distPath}`, { 
    stdio: "inherit"
  });
